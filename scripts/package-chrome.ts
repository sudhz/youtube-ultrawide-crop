/**
 * Package script - creates a Chrome Web Store ready ZIP from dist-chrome/.
...
if (!existsSync(`${distDir}/manifest.json`)) {
  throw new Error('dist-chrome/manifest.json not found. Run `bun run build` first.');
}

const packageJson = JSON.parse(readFileSync('package.json', 'utf8')) as PackageJson;
const name = packageJson.name ?? 'youtube-ultrawide-crop';
const version = packageJson.version ?? '0.0.0';
const outFile = `${artifactsDir}/${name}-v${version}-chrome.zip`;
